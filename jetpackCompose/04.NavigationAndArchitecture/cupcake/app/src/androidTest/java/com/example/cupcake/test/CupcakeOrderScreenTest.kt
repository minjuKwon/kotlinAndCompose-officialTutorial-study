...
class CupcakeOrderScreenTest {
    ...
    private val testOrderUiState=OrderUiState(
        quantity = 6,
        flavor = "Vanilla",
        date="Sta Nov 4",
        price="$100",
    ...
    @Test
    fun selectOptionScreen_optionSelected_NextButtonEnable(){
        val flavors = listOf("Vanilla", "Chocolate", "Hazelnut", "Cookie", "Mango")
        val subtotal="$100"
    ...
    fun summaryScreen_verifyContent() {
        ...
        composeTestRule.setContent {
            OrderSummaryScreen(
                orderUiState = testOrderUiState,
                onCancelButtonClicked = { /*TODO*/ },
                onSendButtonClicked = {_,_ ->}
            )
        }

        composeTestRule.onNodeWithText(testOrderUiState.flavor).assertIsDisplayed()
        composeTestRule.onNodeWithText(testOrderUiState.date).assertIsDisplayed()
        composeTestRule.onNodeWithText(
            composeTestRule.activity.getString(R.string.subtotal_price, testOrderUiState.price)
        ).assertIsDisplayed()
